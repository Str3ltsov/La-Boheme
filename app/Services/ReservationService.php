...
class ReservationService implements ReservationServiceInterface
{
    public function getRandomEmployees(): array|RedirectResponse
    {
        ...
        if (empty($randomEmployees)) {
            return redirect()
                ->route('home')
                ->with('error', __('Failed to find random employees'));
        }
    ...
    public function getReservationTypes(): Collection|RedirectResponse
    {
        ...
        if ($reservationTypes->isEmpty()) {
            return redirect()
                ->route('home')
                ->with('error', __('Failed to find reservation types'));
        }
    ...
    public function getValidationRules(mixed $reservationType): array|RedirectResponse
    {
        ...
        if (empty($validationRules)) {
            return redirect()
                ->route('home')
                ->with('error', __('Failed to find validation rules'));
        }
    ...
    public function getEmployees(): Collection|RedirectResponse
    {
        ...
        if ($employees->isEmpty()) {
            return redirect()
                ->route('home')
                ->with('error', __('Failed to find employees'));
        }
    ...
    : Client|RedirectResponse
    {
        ...
        if ($client->wasRecentlyCreated) {
            ...
            return redirect()
                ->route('home')
                ->with('error', __('Failed to create client'));
        }
    }
    ...
    ): Reservation|RedirectResponse
    {
        ...
        if ($reservation->wasRecentlyCreated) {
            ...
            return redirect()
                ->route('home')
                ->with('error', __('Failed to create reservation'));
        }
    }
    ...
    ): array|RedirectResponse
    {
        ...
        if (empty($answersAndComments)) {
            return redirect()
                ->route('home')
                ->with('error', __('Failed to find reservation question answer and/or comments'));
        }
    ...
    public function getReservationQuestions(int $reservationType): array|RedirectResponse
    {
        if (!$reservationType == null) {
            ...
            if ($reservationQuestions->isEmpty()) {
                return redirect()
                    ->route('home')
                    ->with('error', __('Failed to find reservation questions'));
            }
    ...
    : int|RedirectResponse
    {
        for ($i = 1; $i <= count($questions); $i++) {
            ...
            if (!$reservationQuestionAnswer->wasRecentlyCreated) {
                return redirect()
                    ->route('home')
                    ->with('error', __('Failed to create reservation question answer and/or comment'));
            }
        }
    ...
    public function createReservationEmployees(object $reservation, array $chosenEmployees): int|RedirectResponse
    {
        for ($i = 1; $i <= count($chosenEmployees); $i++) {
            ...
            if (!$reservationEmployee->wasRecentlyCreated) {
                return redirect()
                    ->route('home')
                    ->with('error', __('Failed to create reservation employees'));
            }
        }
